...
> ⚠️ 타인의 대화를 다루는 도구입니다. 쓰기 전에 **[안전 사용 매뉴얼](#안전-사용-매뉴얼-정책법-리스크-최소화)**(카카오 약관·개인정보보호법 준수)을 꼭 읽어 주세요.
...
## 안전 사용 매뉴얼 (정책·법 리스크 최소화)

이 도구는 **타인의 대화가 오가는 오픈채팅방**을 다루므로, 카카오 운영정책과 개인정보보호법(PIPA)을 지키는 선에서 써야 합니다. 아래를 지키면 정책적 리스크가 사실상 없습니다.

### ✅ 해도 되는 것
- **본인이 참여 중인 방**을, **본인 화면에 표시되는 범위**에서, 본인 PC에 로컬로 저장·요약. (PC카톡 기본 '대화 내용 내보내기' 기능 활용 → 카카오 운영정책 위반 아님)
- 요약·검색 결과를 **본인만** 열람.

### ⛔ 하지 말아야 할 것
- **수집한 원문·요약을 제3자에게 공유/게시/전송**하는 행위. 오픈채팅 참여자의 닉네임·발언은 개인정보가 될 수 있어, 공유하려면 **각 정보주체의 동의**가 필요합니다(PIPA). → 본 도구가 외부 전송·업로드 기능을 **의도적으로 넣지 않은** 이유입니다.
- 본인이 참여하지 않은 방을 수집하는 행위.
- 수집물을 영리·마케팅·프로파일링 목적으로 이용하는 행위.

### 📋 권장 사용 수칙
1. **빈도 제한** — 자동화 추출은 **하루 1~2회**로. 과도한 반복 자동화는 카카오 약관상 제재 대상이 될 수 있습니다.
2. **로컬 보관 원칙** — 데이터는 `~/.kakao-summary-mcp/`에만 두고, 클라우드 동기화 폴더(OneDrive/Drive 등)·공유 폴더에 두지 마세요.
3. **공유가 꼭 필요하면 비식별화** — 부득이 일부를 공유해야 하면 닉네임·연락처·식별 가능한 표현을 가린 뒤, 가능한 한 동의를 받으세요.
4. **보관 최소화** — 더 이상 필요 없는 원문/인덱스(`data/`)는 주기적으로 삭제하세요.
5. **이 레포에 데이터 커밋 금지** — `.gitignore`가 `data/`·`logs/`·`rooms.yaml`을 제외하지만, 캡처·내보낸 .txt·오픈채팅 **초대 링크**를 코드/문서/테스트에 넣지 마세요(예시는 `https://open.kakao.com/o/EXAMPLE` 같은 더미만 사용).
6. **방 운영자 정책 존중** — 방 공지나 운영자가 대화 저장·반출을 금지하면 따르세요.

> ⚠️ 본 안내는 법률 자문이 아닙니다. 실제 적용은 본인 책임이며, 거주 지역의 법령·카카오 약관 최신본을 확인하세요. 도구(코드)는 중립적이며, 책임 있는 사용 여부는 전적으로 사용자에게 있습니다.
